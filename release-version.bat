...
:: 创建临时文件来处理替换
powershell -Command "(Get-Content README.md) -replace 'RemoveExtraBlankLines-\d+\.\d+\.\d+\.jar', 'RemoveExtraBlankLines-%VERSION%.jar' -replace '项目版本：\d+\.\d+\.\d+', '项目版本：%VERSION%' | Set-Content README.md"
...
:: 提交README更改（如果有）
git status --porcelain README.md > temp_readme_status.txt
for /f %%i in (temp_readme_status.txt) do (
